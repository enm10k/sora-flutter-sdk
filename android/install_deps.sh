...
SORA_VERSION=2022.11.1

mkdir -p $INSTALL_DIR

if [ ! -e $INSTALL_DIR/webrtc ]; then
  file=webrtc.android-${WEBRTC_VERSION}.tar.gz
  curl -Lo $INSTALL_DIR/$file https://github.com/shiguredo-webrtc-build/webrtc-build/releases/download/${WEBRTC_VERSION}/webrtc.android.tar.gz
  tar -xf $INSTALL_DIR/$file -C $INSTALL_DIR
  # Dart はロングパスをサポートしていない (https://github.com/dart-lang/sdk/issues/27825) ため、
  # ディレクトリが奥深くまである examples ディレクトリは削除する。
  rm -rf $INSTALL_DIR/webrtc/include/examples
fi

if [ ! -e $INSTALL_DIR/boost ]; then
  file=boost-${BOOST_VERSION}_sora-cpp-sdk-${SORA_VERSION}_android.tar.gz
...
if [ ! -e $INSTALL_DIR/sora ]; then
  file=sora-cpp-sdk-${SORA_VERSION}_android.tar.gz
  curl -Lo $INSTALL_DIR/$file https://github.com/shiguredo/sora-cpp-sdk/releases/download/${SORA_VERSION}/${file}
  tar -xf $INSTALL_DIR/$file -C $INSTALL_DIR
fi

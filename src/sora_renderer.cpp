...
Java_jp_shiguredo_sora_1flutter_1sdk_RunOnMainThread_nativeRun(
    JNIEnv* env,
    jobject /* this */,
    jlong ptr) {
...
T InvokeOnMainThread(JNIEnv* env, std::function<T(JNIEnv*)> f) {
  ...
  // r.run();
  auto cls =
      webrtc::GetClass(env, "jp/shiguredo/sora_flutter_sdk/RunOnMainThread");
  jmethodID init_id = env->GetMethodID(cls.obj(), "<init>", "(J)V");
  jmethodID run_id = env->GetMethodID(cls.obj(), "run", "()V");
  webrtc::ScopedJavaLocalRef<jobject> obj(
      env, env->NewObject(cls.obj(), init_id, reinterpret_cast<jlong>(&wf)));
...
#define PIXEL_BUFFER(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), pixel_buffer_get_type(), PixelBuffer))

static gboolean _pixel_buffer_copy_pixels(FlPixelBufferTexture* texture,
                                          const uint8_t** dst,
                                          uint32_t* width,
                                          uint32_t* height,
                                          GError** error) {
...
PixelBuffer* pixel_buffer_new(CopyPixelFunction copy_pixels, void* data) {
  PixelBuffer* buffer =
      PIXEL_BUFFER(g_object_new(pixel_buffer_get_type(), nullptr));
...
#if defined(__ANDROID__)
SoraRenderer::Sink::Sink(webrtc::VideoTrackInterface* track,
                         JNIEnv* env,
...
#endif
{
  ...
  auto texture_entry =
      InvokeOnMainThread<jobject>(env, [texture_registrar](JNIEnv* env) {
        webrtc::ScopedJavaLocalRef<jclass> texcls(
            env, env->GetObjectClass(texture_registrar));
        jmethodID create_surface_id = env->GetMethodID(
            texcls.obj(), "createSurfaceTexture",
            "()Lio/flutter/view/TextureRegistry$SurfaceTextureEntry;");
        webrtc::ScopedJavaLocalRef<jobject> texture_entry(
            env, env->CallObjectMethod(texture_registrar, create_surface_id));
      ...
      std::make_unique<flutter::TextureVariant>(flutter::PixelBufferTexture(
          [this](size_t width,
            ...
            if (pixel_buffer_->width != frame_->width() ||
              ...
              rgba_buffer_.reset(
                  new uint8_t[frame_->width() * frame_->height() * 4]);
  ...
  texture_ = [[SoraRendererTexture alloc] initWithBlock:^{
    ...
    if (pixel_buffer_ == NULL ||
      ...
      if (pixel_buffer_ == NULL) {
        ...
        RTC_LOG(LS_INFO) << " Change PixelBuffer size "
                         << CVPixelBufferGetWidth(pixel_buffer_) << "x"
                         << CVPixelBufferGetHeight(pixel_buffer_) << " to "
                         << frame_->width() << "x" << frame_->height();
      }
      ...
      CVReturn status = CVPixelBufferCreate(
          NULL, frame_->width(), frame_->height(), kCVPixelFormatType_32ARGB,
          (__bridge CFDictionaryRef)options, &pixel_buffer_);
    ...
    libyuv::I420ToARGB(buf->DataY(), buf->StrideY(), buf->DataU(),
                       buf->StrideU(), buf->DataV(), buf->StrideV(), p, pitch,
                       frame_->width(), frame_->height());
...
#elif defined(__linux__)
  texture_.reset(
      FL_TEXTURE(pixel_buffer_new(
          [](void* data, FlPixelBufferTexture* texture, const uint8_t** buffer,
            ...
            if (self->frame_ == nullptr) {
              *error = g_error_new(g_quark_from_static_string("pixel_buffer"),
                                   1, "frame is null");
              return FALSE;
            }
            ...
            if (self->rgba_buffer_ == nullptr ||
              ...
              self->rgba_buffer_.reset(new uint8_t[self->frame_->width() *
                                                   self->frame_->height() * 4]);
              self->width_ = self->frame_->width();
              self->height_ = self->frame_->height();
            }
            ...
            *buffer = self->rgba_buffer_.get();
            ...
            libyuv::I420ToABGR(buf->DataY(), buf->StrideY(), buf->DataU(),
                               buf->StrideU(), buf->DataV(), buf->StrideV(),
                               self->rgba_buffer_.get(),
                               self->frame_->width() * 4, self->frame_->width(),
                               self->frame_->height());

            return TRUE;
          },
          this)),
      [](FlTexture* p) { g_object_unref(p); });
#endif

#if defined(__ANDROID__)
  // texture_id_ = texture_entry_.id();
  webrtc::ScopedJavaLocalRef<jclass> entrycls(
      env, env->GetObjectClass(texture_entry_.obj()));
  ...
  jmethodID surface_texture_id = env->GetMethodID(
      entrycls.obj(), "surfaceTexture", "()Landroid/graphics/SurfaceTexture;");
  webrtc::ScopedJavaLocalRef<jobject> surface_texture(
      env, env->CallObjectMethod(texture_entry_.obj(), surface_texture_id));

  // egl_renderer_ = new EglRenderer("SoraRenderer::Sink");
  auto renderercls = webrtc::GetClass(env, "org/webrtc/EglRenderer");
  jmethodID renderer_ctor_id =
      env->GetMethodID(renderercls.obj(), "<init>", "(Ljava/lang/String;)V");
  webrtc::ScopedJavaLocalRef<jobject> egl_renderer(
      env, env->NewObject(renderercls.obj(), renderer_ctor_id,
                          env->NewStringUTF("SoraRenderer::Sink")));
  ...
  jfieldID config_plain_id =
      env->GetStaticFieldID(basecls.obj(), "CONFIG_PLAIN", "[I");
  webrtc::ScopedJavaLocalRef<jobject> config(
      env, env->GetStaticObjectField(basecls.obj(), config_plain_id));
  ...
  webrtc::ScopedJavaLocalRef<jobject> drawer(
      env, env->NewObject(drawercls.obj(), drawer_ctor_id));

  // egl_renderer_.init(null, config, drawer);
  jmethodID renderer_init_id = env->GetMethodID(
      renderercls.obj(), "init",
      "(Lorg/webrtc/EglBase$Context;[ILorg/webrtc/RendererCommon$GlDrawer;)V");
  //env->CallVoidMethod(egl_renderer_.obj(), renderer_init_id, egl_context.obj(), config.obj(), drawer.obj());
  env->CallVoidMethod(egl_renderer_.obj(), renderer_init_id, nullptr,
                      config.obj(), drawer.obj());

  // egl_renderer_.createEglSurface(surface_texture);
  jmethodID create_egl_surface_id =
      env->GetMethodID(renderercls.obj(), "createEglSurface",
                       "(Landroid/graphics/SurfaceTexture;)V");
  env->CallVoidMethod(egl_renderer_.obj(), create_egl_surface_id,
                      surface_texture.obj());
#elif defined(_WIN32)
...
SoraRenderer::Sink::~Sink() {
  ...
  webrtc::ScopedJavaLocalRef<jclass> renderercls(
      env, env->GetObjectClass(egl_renderer_.obj()));
  jmethodID renderer_release_id =
      env->GetMethodID(renderercls.obj(), "release", "()V");
  env->CallVoidMethod(egl_renderer_.obj(), renderer_release_id);
  RunOnMainThread(env, [this](JNIEnv* env) {
    webrtc::ScopedJavaLocalRef<jclass> entrycls(
        env, env->GetObjectClass(texture_entry_.obj()));
    jmethodID entry_release_id =
        env->GetMethodID(entrycls.obj(), "release", "()V");
...
void SoraRenderer::Sink::OnFrame(const webrtc::VideoFrame& frame) {
...
#if defined(__ANDROID__)
  if (frame_->video_frame_buffer()->type() ==
  ...
  if (width_ != frame.width() || height_ != frame.height()) {
    ...
    webrtc::ScopedJavaLocalRef<jclass> entrycls(
        env, env->GetObjectClass(texture_entry_.obj()));
    jmethodID surface_texture_id =
        env->GetMethodID(entrycls.obj(), "surfaceTexture",
                         "()Landroid/graphics/SurfaceTexture;");
    webrtc::ScopedJavaLocalRef<jobject> surface_texture(
        env, env->CallObjectMethod(texture_entry_.obj(), surface_texture_id));
    webrtc::ScopedJavaLocalRef<jclass> surfcls(
        env, env->GetObjectClass(surface_texture.obj()));
    jmethodID set_size_id =
        env->GetMethodID(surfcls.obj(), "setDefaultBufferSize", "(II)V");
    env->CallVoidMethod(surface_texture.obj(), set_size_id, frame.width(),
                        frame.height());
    width_ = frame.width();
  ...
  webrtc::ScopedJavaLocalRef<jobject> jframe =
      webrtc::NativeToJavaVideoFrame(env, *frame_);
  webrtc::ScopedJavaLocalRef<jclass> renderercls(
      env, env->GetObjectClass(egl_renderer_.obj()));
  jmethodID on_frame_id = env->GetMethodID(renderercls.obj(), "onFrame",
